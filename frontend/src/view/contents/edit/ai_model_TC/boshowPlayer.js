...
class TcCheckPlayer extends Component {
  ...
  async urlSet() {
    ...
    await axios
      .get("/TCTimeApi", { params: { videoIdx: video_idx } })
      .then((response) => {
        if (response.status == 200) {
          tcDetail = response.data;
          for (let item of tcDetail) {
  ...
  addAItem = (ad) => {
    const aType = this.state.videoAdbs.filter((adb) => {
      return adb.idx == ad.fk_adb_idx;
    });
  ...
  addBItem = (ad) => {
    const bType = this.state.videoAdbs.filter((adb) => {
      return adb.idx == ad.fk_adb_idx;
    });
  ...
  async addCItem(ad) {
    const cType = this.state.videoAdbs.filter((adb) => {
      return adb.idx == ad.fk_adb_idx;
    });
  ...
  addDItem(ad) {
    ...
    const tcItems = this.state.tcDetail.map((tc) => {
      return tc.fk_item_idx;
    })[0];

    const items = this.state.videoItems.filter((item) => {
      return tcItems.includes(String(item.idx));
    });

    this.setState({
      adb_d_list: [...items, cType[0]],
    });
  }
  ...
  runBoshowInterval() {
    this.getItemDetail = setInterval(() => {
      if (this.video || this.video != null) {
        ...
        const prevTime = currentTime - 0.366666666666666666666665;
        const futureTime = currentTime + 0.366666666666666666666665;

        if (!this.state.tcDetail) {
          return;
        }

        const itemList =
          this.state.tcDetail &&
          this.state.tcDetail.filter((data) => {
        ...
        this.state.videoAdbMatching.map((adb) => {
          switch (adb.adb_type) {
            case 0:
              this.addAItem(adb);
              break;
  ...
  render() {
    ...
    const aTypeAd = (
      <div>
        <AdbTypeA
          adb_idx={this.state.aType.idx}
          adb_url={this.state.aType.url}
        />
      </div>
    );
    ...
    const d_list = (
      <div>
        {this.state.adb_d_list &&
          this.state.adb_d_list.map((adD, i) => (
            <SwiperSlide
              key={i}
              style={{
                width: "150px",
