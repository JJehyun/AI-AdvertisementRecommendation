...
class _netD_result(nn.Module):
    def __init__(self, ngpu, num_classes=10):
        super(_netD_result, self).__init__()
        self.ngpu = ngpu
        # Convolution 1
    ...
    def forward(self, input):
        ...
        realfake = self.sigmoid(fc_dis).view(-1, 1).squeeze(1) # 0~1값을 출력
...
"""
Convolution
layer 값도 조정(너무 커도, 작아도 좋지 않음)
pre-trained model
LeakyReLU => 
Dropout => 
기본적인 인공 신경망 output과 gradient loss 간의 차이(=loss)를 구한 뒤, backpropagtion 알고리즘을 구하여 weight를 업데이트하는 방식

뉴런이라고 부르는 노드를 무작위로 껐다 켰다를 반복하는 것을 dropout이라함.
Train: 학습할 때는 무작위로 노드를 선택하여 선별적으로 노드를 활용함
Test: 평가할 때는 모든 노드를 사용하겠다는 의미
model.eval()도 사용해야함
...
class _netD_3(nn.Module):
    def __init__(self, ngpu, num_classes=10):
        super(_netD_3, self).__init__()
        self.ngpu = ngpu
        # Convolution 1
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 16, 3, 2, 1, bias=False),
...
class _netD_7(nn.Module):
    def __init__(self, ngpu, num_classes=10):
        super(_netD_7, self).__init__()
        self.ngpu = ngpu
        # Convolution 1
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 16, 3, 2, 1, bias=False),
        ...
        self.softmax = nn.Softmax()
        self.sigmoid = nn.Sigmoid()
        print(self.softmax, ", ", self.sigmoid)
